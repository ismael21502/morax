...
function Positions({ joints, setJoints, setLogs, loadPositions, positions, isConnected, opening, setOpening, coords, setCoords }) {
                ...
                if (data.status === "ok") {
                    ...
                    setLogs((prev) => [
                        ...prev,
                        {
                            ...
                            type: "INFO",
                            values: `La posición fue eliminada con éxito.`,
                        },
                    ])
                } else {
                    setLogs((prev) => [
                        ...prev,
                        {
                            time: new Date().toLocaleTimeString(),
                            type: "ERROR",
                            values: `Error del servidor: ${data.status}`,
                        },
                    ]);
                }
            })
            .catch((err) =>
                setLogs((prev) => [
                    ...prev,
                    {
                        time: new Date().toLocaleTimeString(),
                        type: "ERROR",
                        values: `Error al intentar borrar la posición: ${err}`,
                    },
                ])
            );
    ...
    function moveRobot(targetJoints, targetGripperOpening, targetCoords) {
        ...
        const initialJoints = { ...joints }; // suponiendo que joints es el estado actual
        const initialOpening = opening
        const initialCoords = {...coords}
        function animate(time) {
            const elapsed = time - start;
            const t = Math.min(elapsed / duration, 1); // valor entre 0 y 1

            // Interpolación lineal de cada articulación
            const newJoints = {};
            let newOpening = 0
            const newCoords = {}
            ...
            newOpening = Math.round(initialOpening + t * (targetGripperOpening - initialOpening))
    ...
    function sendPos(e) {
        ...
        if (target) {
            moveRobot(target.joints, target.gripperOpening, target.coords)
        }
    }
    function savePos(e) {
        if (newPosName != "") {
            fetch("http://localhost:8000/save", {
                method: "POST",
                headers: { "Content-Type": "application/json" },
                body: JSON.stringify({ 'name': newPosName, 'joints': joints, 'gripperOpening': opening, 'coords': coords}),
            })
                .then((data) => (data.ok == true) ? [ loadPositions() , setLogs(prev => [...prev, { time: new Date().toLocaleTimeString(), type: "INFO", values: "Posición guardada correctamente" }]) ]: setLogs(prev => [...prev, { time: new Date().toLocaleTimeString(), type: "ERROR", values: "La posición no fue guardada" }]))
                .then(loadPositions())
                .catch((err) => setLogs(prev => [...prev, { time: new Date().toLocaleTimeString(), type: "ERROR", values: `No fue posible guardar la posición: ${err}` }]))
        }
        setNewPosName("")
    }
    function handleNewPos(e) {
    ...
    return (
        <div className='w-[max(330px,25vw)] rounded-lg text-white bg-[#1F1F1F] border-1 border-solid border-[#4A4A4A]'>
            <div className='w-full rounded-t-lg bg-[#2B2B2B] py-2 px-5 font-bold text-xl'>Posiciones</div>
            <div className='p-2 flex flex-col gap-5'>
                            ...
                            {positions.length > 0 ?
                                positions.map((pos) => (
                                    <Select.Item
                                        key={pos.name}
                                        value={pos.name}
                                        className="px-3 py-1 hover:bg-[#3B3939] cursor-pointer"
                                    >
                ...
                <div>
                    ...
                    <div className='flex justify-between'>
                        <input type="text" className='border-1 border-[#3B3939] rounded-sm px-2 w-[8rem]' placeholder='Nombre' onChange={handleNewPos} value={newPosName} />
